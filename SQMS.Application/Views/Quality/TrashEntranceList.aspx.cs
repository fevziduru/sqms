...
namespace SQMS.Application.Views.Quality
{
        ...
        public partial class TrashEntranceList : SQMSPage<MonitorPointService>
        {
            ...
            {
                this.ViewData = srv.GetListDataByType("trashentrance");//Service.LoadByCondition("organizationid='" + CurrentUser.OrganizationID + "'");
            }
                    ...
                    case "Select":
                        {
                            DataKey key = this.gvList.DataKeys[Convert.ToInt32(e.CommandArgument)];
                            string id = ConvertUtil.ToStringOrDefault(key.Value);
                            Response.Redirect("TrashEntranceEdit.aspx?p=trashentranceedit&id=" + id);
                            break;
                        }
                    case "Sort":
            ...
            public void btnDelete_OnClick(object sender, EventArgs e)
            {
                ...
                //删除数据后重新加载数据
                this.ViewData = srv.GetListDataByType("trashentrance");
                this.gvList.DataSource = this.ViewData;
                this.gvList.DataBind();
            }
