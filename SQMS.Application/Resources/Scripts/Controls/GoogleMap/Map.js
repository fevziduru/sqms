...
function initMap() {
        if (GBrowserIsCompatible()) {
                if (!map) {
                        document.getElementById("map_canvas").style.height = document.body.clientHeight - 100;
                        ...
                        GEvent.addListener(map.getInfoWindow(), "maximizeend", function() {
                                ...
                                if (0 == selectedTabIndex) {
                                        url = "/Views/Quality/MonitorPointDetail.aspx?p=__pub__&qcType=_qc_type_normal&mpid=" + currentClickedMarker.mpId;
                                } else if (1 == selectedTabIndex) {
                                        url = "/Views/Quality/MonitorPointDetail.aspx?p=__pub__&qcType=_qc_type_dynamic&mpid=" + currentClickedMarker.mpId;
                                }
...
function fetchMarkers() {
        ...
        var url = "/Views/AjaxServices/QualityControl/MonitorPoint.aspx?p=qualityinfo&swlat=" + bound.getSouthWest().lat()
