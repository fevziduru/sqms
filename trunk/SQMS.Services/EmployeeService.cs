...
namespace SQMS.Services
{
        public class EmployeeService : GenericService
        {
                ...
                {
                        //将其它职员的设备绑定信息设置为无效
                        string equid = ConvertUtil.ToStringOrDefault(DataSetUtil.GetFirstRowFromDataSet(dsSave, BOName)["EQUID"]);
                        DefaultSession.ExecuteCommand("update employee set isequactivate='N' where equid=:equid and organizationid=:orgid", equid, CurrentUser.OrganizationID);
