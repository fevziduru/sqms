...
namespace SQMS.Application.Views.Quality
{
        ...
        public partial class AccidentList : SQMSPage<MonitorPointService>
        {
            ...
            {
                this.ViewData = srv.GetListDataByType("accident");//Service.LoadByCondition("organizationid='" + CurrentUser.OrganizationID + "'");
            }
                    ...
                    case "Select":
                        {
                            DataKey key = this.gvList.DataKeys[Convert.ToInt32(e.CommandArgument)];
                            string id = ConvertUtil.ToStringOrDefault(key.Value);
                            Response.Redirect("AccidentEdit.aspx?p=accidentedit&id=" + id);
                            break;
                        }
                    case "Sort":
            ...
            public void btnDelete_OnClick(object sender, EventArgs e)
            {
                ...
                //删除数据后重新加载数据
                this.ViewData = srv.GetListDataByType("accident");
                this.gvList.DataSource = this.ViewData;
                this.gvList.DataBind();
            }
