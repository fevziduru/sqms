...
namespace SQMS.Application.Views.Basedata
{
    ...
    public partial class OrganizationList : SQMSPage<OrganizationService>
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                this.gvList.Attributes.Add("SortExpression", "ORGID");
                ...
                case "Select":
                    {
                        DataKey key = this.gvList.DataKeys[Convert.ToInt32(e.CommandArgument)];
                        string id = ConvertUtil.ToStringOrDefault(key.Value);
                        Response.Redirect("OrganizationEdit.aspx?p=organizationedit&id=" + id);
                        break;
                    }
                case "Sort":
        ...
        public void btnDelete_OnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //设置引用到它的节点
                    (Service as OrganizationService).SetParent2null(ids[i]);
            ...
            DataTable dt = DataSetUtil.GetDataTableFromDataSet(this.ViewData, "ORGANIZATION");
            this.OrganizationTree1.RefreshMenu(dt);
        }
