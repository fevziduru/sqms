...
namespace SQMS.Application.Views.Vehicle
{
    ...
    public partial class VehicleTaskList : SQMSPage<VehicleTaskService>
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                this.gvList.Attributes.Add("SortExpression", "TASKID");
        ...
        {
            this.ViewData = srv.LoadAll();
        }

        protected override void OnInitializeViewEventHandler(object sender, EventArgs e)
        {
            this.gvList.DataSource = this.ViewData.Tables[srv.BOName].DefaultView;
                ...
                case "Select":
                    {
                        DataKey key = this.gvList.DataKeys[Convert.ToInt32(e.CommandArgument)];
                        string id = ConvertUtil.ToStringOrDefault(key.Value);
                        Response.Redirect("VehicleTaskEdit.aspx?p=vtedit&id=" + id);
                        break;
                    }
                case "Sort":
