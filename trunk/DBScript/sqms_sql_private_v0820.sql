...
ALTER TABLE MONITORPOINTINVIDEO
   DROP CONSTRAINT FK_MONITORP_FK_MONITO_QUALITYD;
...
DROP INDEX IDX_QUALITYDATA_CREATED;

DROP INDEX IDX_QUALITYDATA_ORGANIZATIONID;

DROP TABLE QUALITYDATA CASCADE CONSTRAINTS;
...
CREATE TABLE QUALITYDATA  (
   ...
   QUALITYDATATYPE      VARCHAR2(40),
   LONGITUDE            NUMBER(17,14),
   LATITUDE             NUMBER(17,14),
   CONSTRAINT PK_QUALITYDATA PRIMARY KEY (VIDEOID)
);

COMMENT ON TABLE QUALITYDATA IS
'质量监控时采集的视频数据';

COMMENT ON COLUMN QUALITYDATA.VIDEOID IS
'视频ID';

COMMENT ON COLUMN QUALITYDATA.MPID IS
'监控点ID';

COMMENT ON COLUMN QUALITYDATA.VIDEONAME IS
'视频名称';

COMMENT ON COLUMN QUALITYDATA.MEMO IS
'备注';

COMMENT ON COLUMN QUALITYDATA.VIDEOURL IS
'视频位置';

COMMENT ON COLUMN QUALITYDATA.TRACE IS
'视频轨迹';

COMMENT ON COLUMN QUALITYDATA.CREATED IS
'创建时间';

COMMENT ON COLUMN QUALITYDATA.CREATEDBY IS
'创建人';

COMMENT ON COLUMN QUALITYDATA.MODIFIED IS
'修改时间';

COMMENT ON COLUMN QUALITYDATA.MODIFIEDBY IS
'修改人';

COMMENT ON COLUMN QUALITYDATA.ORGANIZATIONID IS
'ORGANIZATIONID';

COMMENT ON COLUMN QUALITYDATA.SUITEID IS
'SUITEID';

COMMENT ON COLUMN QUALITYDATA.IMAGEURL IS
'图像位置';

COMMENT ON COLUMN QUALITYDATA.QUALITYDATATYPE IS
'质量数据类型';

COMMENT ON COLUMN QUALITYDATA.LONGITUDE IS
'经度';

COMMENT ON COLUMN QUALITYDATA.LATITUDE IS
'纬度';

/*==============================================================*/
/* Index: IDX_QUALITYDATA_ORGANIZATIONID                        */
/*==============================================================*/
CREATE INDEX IDX_QUALITYDATA_ORGANIZATIONID ON QUALITYDATA (
   ORGANIZATIONID ASC
);

/*==============================================================*/
/* Index: IDX_QUALITYDATA_CREATED                               */
/*==============================================================*/
CREATE INDEX IDX_QUALITYDATA_CREATED ON QUALITYDATA (
   CREATED DESC
);
...
ALTER TABLE QUALITYDATA
   ADD CONSTRAINT FK_QUALITYD_REFERENCE_MPASSIGN FOREIGN KEY (MPID)
      REFERENCES MPASSIGNMENT (MPID);

ALTER TABLE QUALITYDATA
   ADD CONSTRAINT FK_QUALITYD_REFERENCE_ENUMERAT FOREIGN KEY (QUALITYDATATYPE)
      REFERENCES ENUMERATION (ENUMID);
